...
import me.wackee.al.menusystem.PaginatedMenu;
...
public class KillPlayerMenu extends PaginatedMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getType().equals(Material.PLAYER_HEAD)) {
            ...
            PlayerMenuUtility playerMenuUtility = Al.getPlayerMenuUtility(p);
            playerMenuUtility.setPlayerToKill(Bukkit.getPlayer(UUID.fromString(e.getCurrentItem().getItemMeta().getPersistentDataContainer().get(new NamespacedKey(Al.getPlugin(), "uuid"), PersistentDataType.STRING))));
        ...
        if(players != null && !players.isEmpty()) {
            for(int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (players.get(index) != null){
                    ...
                    playerMeta.getPersistentDataContainer().set(new NamespacedKey(Al.getPlugin(), "uuid"), PersistentDataType.STRING, players.get(index).getUniqueId().toString());
